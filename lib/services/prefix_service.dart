...
class PrefixService {
  static CollectionReference<Map<String, dynamic>> _baseCollection({String? clinicIdOverride}) {
    ...
    if (FeatureFlags.useNestedCollections && clinicId != null && clinicId.isNotEmpty) {
      return firestore.collection('clinics').doc(clinicId).collection('prefix_master');
    }
    return firestore.collection('prefix_master').withConverter<Map<String, dynamic>>(
  ...
  static Stream<List<Prefix>> getAllPrefixes({String? clinicIdOverride}) {
    return _baseCollection(clinicIdOverride: clinicIdOverride).snapshots().map((snapshot) {
  ...
  static Future<void> addIfNotExist(String name, {String? clinicIdOverride}) async {
  ...
  static Future<void> updatePrefix(String id, String name, {String? clinicIdOverride}) async {
    final col = _baseCollection(clinicIdOverride: clinicIdOverride);
    await col.doc(id).update({'name': name.trim()});
  }

  static Future<void> deletePrefix(String id, {String? clinicIdOverride}) async {
    final col = _baseCollection(clinicIdOverride: clinicIdOverride);
